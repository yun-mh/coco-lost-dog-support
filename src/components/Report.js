...
const Report = ({ report, token, dogId, user }) => {
  ...
  const getCoordinates = async () => {
    try {
      const res = await fetch(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${report.location}components=country:JP&key=${process.env.REACT_APP_GOOGLE_MAPS_API_KEY}`
      );
  ...
  return (
    <ReportContainer>
      ...
      <ReportContentContainer>
        <ReportHeader>
          <span className="text-blue-600 font-semibold">{report.name}</span>
          さんがレポートしました。
        </ReportHeader>
        <ReportDetail>
          <div>
            ...
            <span className="text-blue-600">
              {moment(report.when).format("YYYY年MM月D日ah時mm分")}
            </span>
            基準、
          ...
          <div>
            ...
            <div className="text-gray-600">
              名前：
              <span className="font-semibold text-blue-600">{report.name}</span>
            </div>
            <div className="text-gray-600">
              連絡先：
            ...
            <div className="text-gray-600">
              メモ：
              <span className="font-semibold text-blue-600">{report.memo}</span>
            </div>
          </div>
        </ReportDetail>
        <ReportFooter>
          {moment(report.createdAt).format("YYYY-MM-D ah:mm:ss")}
        </ReportFooter>
        <Button
          type="button"
          use="accent"
          title="レポート修正"
          onClick={openRModifyModal}
        />
      </ReportContentContainer>
